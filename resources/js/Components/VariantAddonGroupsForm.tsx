        ...
        <div className="mt-3">
            {variantAddonGroupList.map((group) => (
                <div key={group.id} className={`mb-2 border border-gray-200 p-2 rounded-lg ${(selectedAddonGroupOptions
                    .filter(sa => sa.option.addon_group_id === group.id).length < 1) ? '' : (selectedAddonGroupOptions
                    .filter(sa => sa.option.addon_group_id === group.id).reduce((sum, sa) => sum + sa.qty, 0) < (group.min_options || 0)) ? 'border-red-500' : 'border-green-500'}`}>
                    ...
                    <h3 className="text-lg font-semibold mb-1">{group.name}:
                        {group.is_required && (
                            <span className="text-sm text-red-500 font-normal"> * Mínimo {group.min_options}, máximo {group.max_options ?? 'ilimitado'}</span>
                        )}
                    </h3>
                    ...
                    <div className="space-y-1">
                        {group.addon_group_options?.map((addonGroupOption) => {
                            ...
                            return (
                                <div key={addonGroupOption.id} className="flex items-center justify-between">
                                    <div>
                                        <span className="font-medium">{addonGroupOption.addon?.name}</span>
                                        {addonGroupOption.additional_price > 0 && (
                                            <span className="text-sm text-gray-500"> (+ R$ {addonGroupOption.additional_price})</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex items-center bg-neutral-50 border border-neutral-200 rounded-lg">
                                        <button
                                            ...
                                            onClick={() => handleAddonChange(group.id!, addonGroupOption.id, (selected?.qty || 0) - 1)}
                                            aria-label="Diminuir"
                                        >-</button>
